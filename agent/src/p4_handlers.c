...
int insert_newlink(struct link_message *lm)
{
    ...
    spec.pipe = 0;
    spec.table_name = "ingress_src_mac_resolve";
...
int insert_newroute(struct route_message *rm)
{
    ...
    if (rm->vrf == RT_TABLE_LOCAL) {
        if (rm->is_ip4) {
            ...
            spec.table_name = "ingress_forward_local_table_ip6";
            spec.action_name = "ingress_normal_path_to_kernel";
            spec.num_elems = 1;
            spec.num_data = 0;

            key[0].binary_val = rm->addr6;
            key[0].type = IP6_ADDR;
        }
    } else {
        if (rm->is_ip4) {
            ...
            spec.table_name = "ingress_routing_table_ip6";
            spec.action_name = "ingress_forward_ip6";
            spec.num_elems = 2;
            spec.num_data = 2;

            key[0].val = rm->vrf;
            key[0].type = U_32;
            key[1].binary_val = rm->addr6;
            key[1].prefix_len = rm->prefix_len;
            key[1].type = IP6_LPM;

            action_data[0].val = rm->dev;
            action_data[0].type = U_32_DATA;
            action_data[1].binary_val = rm->gw6;
            action_data[1].type = IP6_DATA;
        }
    }
...
int insert_newneigh(struct neigh_message *nm)
{
    ...
    if (nm->is_ip4)
        ...
        inet_ntop(AF_INET6, nm->gw6, gw, IP_BUF_SIZE);
    ...
    if (nm->is_ip4)
        ...
        spec.table_name = "ingress_arp_resolve_ip6";
